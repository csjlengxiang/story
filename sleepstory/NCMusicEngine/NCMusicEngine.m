...
- (void)handlePlayCheckingTimer:(NSTimer *)timer {
  ...
  if (self.delegate &&
      ...
      [self.delegate respondsToSelector:@selector(engine:playProgress:current:duration:)]) {
    if (playerDuration <= 0)
      [self.delegate engine:self playProgress:0 current:0 duration:0];
    else
      [self.delegate engine:self playProgress:playerCurrentTime / playerDuration current:playerCurrentTime duration:playerDuration];
  }
...
- (void)audioPlayerDidFinishPlaying:(AVAudioPlayer *)player successfully:(BOOL)flag {
  if (flag) {
    ...
    if (self.delegate &&
        [self.delegate conformsToProtocol:@protocol(NCMusicEngineDelegate)] &&
        [self.delegate respondsToSelector:@selector(engine:playProgress:current:duration:)]) {
//      [self.delegate engine:self playProgress:1.f current:<#(CGFloat)#> duration:<#(CGFloat)#>];
    }
  }
}
